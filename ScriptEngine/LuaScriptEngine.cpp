...
#include "LuaScriptEngine_wrap.cxx"
...
extern int luaopen_ScriptEngine(lua_State* L);
...
static const luaL_reg lualibs[] = {
  ...
  {"ScriptEngine", luaopen_ScriptEngine},
  LUA_EXTRALIBS
  {NULL, NULL}
};
...
template<class LuaScriptEngine> LuaScriptEngine* Singleton<LuaScriptEngine>::ms_Singleton = NULL;
/**
 * Run the given script.
 */
int LuaScriptEngine::runScript( const char *fileName ) {
    /* run the script */
    return lua_dofile( m_pLuaVM, fileName );
}
...
int LuaScriptEngine::runString(const char *strLuaInput ) {
    /* run the string */
   return lua_dostring(m_pLuaVM, strLuaInput);
}

int LuaScriptEngine::loadBuffer(const char *buff, char *msg) {

  char *charmsg;
  int error = luaL_loadbuffer(m_pLuaVM, buff, strlen(buff), "line");
  if (error) {
    sprintf(charmsg, "%s", lua_tostring(m_pLuaVM, -1));
