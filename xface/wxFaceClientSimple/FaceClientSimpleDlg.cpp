...
BEGIN_EVENT_TABLE(FaceClientSimpleDlg, wxDialog)
		EVT_CLOSE(FaceClientSimpleDlg::OnClose)
		EVT_SOCKET(ID_SOCKET,  FaceClientSimpleDlg::OnSocketEvent)
		EVT_BUTTON(ID_CONNECT, FaceClientSimpleDlg::OnConnect)
		EVT_BUTTON(ID_DISCONNECT, FaceClientSimpleDlg::OnDisconnect)
		EVT_BUTTON(ID_PLAY, FaceClientSimpleDlg::OnPlay)
		EVT_BUTTON(ID_KILL, FaceClientSimpleDlg::OnKill)
		EVT_BUTTON(ID_SEND_SCRIPT, FaceClientSimpleDlg::OnSendScript)
END_EVENT_TABLE()
											   ...
											   : wxDialog(parent, id, title, pos, size, style, name)
{
	...
	topsizer->Add(new wxButton(this, ID_CONNECT, _("Connect")), 0, wxALL | wxALIGN_CENTER, 10);
	topsizer->Add(new wxButton(this, ID_SEND_SCRIPT, _("Send SMIL-Agent Script")), 0, wxALL | wxALIGN_CENTER, 10);
	topsizer->Add(new wxButton(this, ID_PLAY, _("Play")), 0, wxALL | wxALIGN_CENTER, 10);
	topsizer->Add(new wxButton(this, ID_DISCONNECT, _("Disconnect")), 0, wxALL | wxALIGN_CENTER, 10);
	topsizer->Add(new wxButton(this, ID_KILL, _("Quit Client")), 0, wxALL | wxALIGN_CENTER, 10);
...
void FaceClientSimpleDlg::Connect(bool bConnect)
{
	...
	if(bConnect) // establish connection
	{
		...
		wxLogMessage(wxString::Format(_("Connecting... to %s"), m_IPAddress));
		...
		if(m_pSocket->Connect(addr, TRUE))
		  ...
		  wxLogMessage(wxString::Format(_("Cannot establish connection to %s!"), m_IPAddress));
	}
	else
	{
		if(!m_bConnected)
	...
	switch(event.GetSocketEvent())
	{
		...
		case wxSOCKET_INPUT: // gets only notifications
		{
			...
			if(note.getName() == "CONNECTION_OK")
			{
				...
				m_bConnected = true;
				wxLogMessage(wxString::Format(_("Connected successfully to server with ClientID %d."), m_clientId));
			}
			else
				OnNotification(note);
...
void FaceClientSimpleDlg::OnNotification(const XFaceApp::Notification& note)
{
	if(note.getStatus() == XFaceApp::Notification::kQueued)
	  wxLogMessage(wxString::Format(_("Task %s sent and queued by server successfully (TaskID=%d)"), note.getName().c_str(), note.getTaskID()));
	else if(note.getStatus() == XFaceApp::Notification::kError)
	  wxLogMessage(wxString::Format(_("An error occured while sending task %s!"), note.getName().c_str()));
	else if(note.getStatus() == XFaceApp::Notification::kStarted)
	  wxLogMessage(wxString::Format(_("%s status: STARTED received for task %d"), note.getName().c_str(), note.getTaskID()));
	else if(note.getStatus() == XFaceApp::Notification::kFinished)
	  wxLogMessage(wxString::Format(_("%s status: FINISHED received for task %d"), note.getName().c_str(), note.getTaskID()));
	else	
	  wxLogMessage(wxString::Format(_("%s status:%d received for task %d"), note.getName().c_str(), (int)note.getStatus(), note.getTaskID()));
}
...
bool FaceClientSimpleDlg::SendTask(const XFaceApp::Task& task)
{
	...
	wxLogMessage(wxString::Format(_("Task %s with id %d sent"), task.getName().c_str(), task.getID()));
